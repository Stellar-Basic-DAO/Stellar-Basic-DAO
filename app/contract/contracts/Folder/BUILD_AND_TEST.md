...
cargo build --package stellar_basic_dao

# Build with release optimizations
cargo build --release --package stellar_basic_dao
```
...
```bash
# Run all tests
cargo test --package stellar_basic_dao
...
cargo test --package stellar_basic_dao -- --include-ignored
```
...
```
Compiling stellar_basic_dao v0.1.0 (...)
Finished `dev` profile [unoptimized + debuginfo] target(s) in XX.XXs

Running unittests src/lib.rs (target/debug/deps/stellar_basic_dao-xxxxxxxxxxxxxxx)
