...
impl RustAcademyContract {
    ...
    // -----------------------------------------------------------------------
    // DAO Governance (governance-model-v1)
    // -----------------------------------------------------------------------

    /// Create a governance proposal for a privileged action.
    ///
    /// The proposer is automatically recorded as the first approval.
    /// Replay protection is enforced via per-signer nonces.
    ///
    /// # Arguments
    /// * `proposer` — must be a governance signer; must authorize
    /// * `action` — the privileged action to gate
    /// * `nonce` — unique u64 per signer (consumed on success)
    /// * `valid_until` — expiry timestamp (must be in the future; max 30 days)
    ///
    /// # Errors
    /// * `NotASigner`, `SignatureExpired`, `ExpiryTooFar`, `NonceAlreadyUsed`, `ProposalAlreadyExists`
    ...
    ///
    /// When the approval count reaches the threshold, the proposal becomes `Executable`.
    ...
    ///
    /// Only callable by a governance signer.
    ...
    /// Get the current governance approval threshold.
    pub fn get_governance_threshold(env: Env) -> u32 {
        governance::get_threshold(&env)
    }

    /// Get a governance proposal by its proposal_id.
    ...
    /// Check whether an address is a governance signer.
