...
pub fn batch_create(
    ...
    for (i, item) in items.iter().enumerate() {
        ...
        if get_escrow(env, &item.escrow_id).is_some() {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  StellarBasicDAOError::EscrowExists as u32 });
            continue;
        }
...
pub fn batch_release(
    ...
    for (i, id) in escrow_ids.iter().enumerate() {
        ...
        let mut entry = match get_escrow(env, &id) {
            Some(e) => e,
            None => {
                results.push_back(BatchItemResult { index: idx, success: false, error_code:  StellarBasicDAOError::EscrowNotFound as u32 });
                continue;
            }
        };
