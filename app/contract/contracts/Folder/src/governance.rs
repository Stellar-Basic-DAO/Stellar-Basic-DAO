...
fn apply_action(env: &Env, action: &ProposalAction) -> Result<(), GovernanceError> {
    match action {
        ...
        ProposalAction::SetPauseFlags(enable_mask, disable_mask) => {
            ...
            crate::storage::set_pause_flags(env, &zero, *enable_mask, *disable_mask);
            Ok(())
        }
        ...
        ProposalAction::GrantRole(target, role) => {
            ...
            let mut roles = crate::storage::get_roles(env, target);
            if !roles.contains(r) {
                roles.push_back(r);
                crate::storage::set_roles(env, target, &roles);
            }
            Ok(())
        }
        ProposalAction::RevokeRole(target, role) => {
            use crate::types::Role;
            let r = u32_to_role(*role)?;
            let old_roles = crate::storage::get_roles(env, target);
            ...
            crate::storage::set_roles(env, target, &new_roles);
            Ok(())
        }
